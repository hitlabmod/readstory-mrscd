...
export default async function serialize(wilykun, msg, store) {
	const m = {};
	...
	if (msg.key) {
		...
		m.sender = jidNormalizedUser(m.fromMe ? wilykun.user.id : m.isGroup ? m.participant : m.from);
	}

	if (m.isGroup) {
		if (!(m.from in store.groupMetadata)) store.groupMetadata[m.from] = await wilykun.groupMetadata(m.from);
		...
		m.isBotAdmin = m.isGroup && !!m.groupAdmins.find(member => member.id === jidNormalizedUser(wilykun.user.id));
	}
		...
		if (m.msg?.contextInfo?.quotedMessage) {
			...
			if (m.quoted.message) {
				...
				m.quoted.key = {
					...
					fromMe: areJidsSameUser(jidNormalizedUser(m.msg?.contextInfo?.participant), jidNormalizedUser(wilykun?.user?.id)),
	...
	m.reply = async (text, options = {}) => {
		if (typeof text === 'string') {
			return await wilykun.sendMessage(m.from, { text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		} else if (typeof text === 'object' && typeof text !== 'string') {
			return wilykun.sendMessage(m.from, { ...text, ...options }, { quoted: m, ephemeralExpiration: m.expiration, ...options });
		}
	};
